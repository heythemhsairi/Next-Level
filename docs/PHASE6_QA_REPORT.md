...
- Dashboard: removed the duplicate overdue summary; hid empty momentum tiles; made the welcome and sidebar calmer; ensured animated counts show their real value in background tabs. Client and task lists now have mobile cards, and the finance page replaces four zero-value tiles with a useful empty state. Portal home and navigation were made more compact, and the content calendar uses only the weeks needed for a month. Added portal loading and error states.
